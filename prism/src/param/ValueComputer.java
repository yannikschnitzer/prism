...
//		}
	...
	private MutablePMC buildAlterablePMCForReach(DTMC<Function> dtmc, StateValues b1, StateValues b2, MCRewards<Function> mcRewards)
	{
		MutablePMC pmc = new MutablePMC(functionFactory, dtmc.getNumStates(), mcRewards != null, false);
		for (int state = 0; state < dtmc.getNumStates(); state++) {
			...
			pmc.setInitState(state, dtmc.isInitialState(state));
			if (isSink) {
				...
				if (mcRewards != null) {
					pmc.setReward(state, mcRewards.getStateReward(state));
				}
				Function sum = functionFactory.getZero();
				Iterator<Entry<Integer, Function>> iter = dtmc.getTransitionsIterator(state);
				while (iter.hasNext()) {
					Entry<Integer, Function> e = iter.next();
					sum = sum.add(e.getValue());
				}
				iter = dtmc.getTransitionsIterator(state);
				while (iter.hasNext()) {
					Entry<Integer, Function> e = iter.next();
					pmc.addTransition(state, e.getKey(), e.getValue().divide(sum));
				}
			}
		}
	...
	private MutablePMC buildAlterablePMCForSteady(DTMC<Function> dtmc, StateValues b1, MCRewards<Function> rew) throws PrismException
	{
		// TODO
		throw new PrismNotSupportedException("No SS");
		/*
		MutablePMC pmc = new MutablePMC(functionFactory, dtmc.getNumStates(), true, true);
		for (int state = 0; state < dtmc.getNumStates(); state++) {
			pmc.setTargetState(state, false);
			pmc.setInitState(state, dtmc.isInitialState(state));
			Function sumLeaving = dtmc.sumLeaving(state); // CHANGED
			if (rew != null) {
				pmc.setReward(state, rew.getStateReward(state));
			} else {
			...
			while (iter.hasNext()) {
				Entry<Integer, Function> e = iter.next();
				pmc.addTransition(state, e.getKey(), e.getValue());
			}
	...
	private RegionValues computeSteadyState(Model<?> model, Region region, StateValues b1, boolean min, Rewards<?> rew) throws PrismException
	{
		...
		DTMC<Function> dtmc = (DTMC<Function>) model;
		...
		StateValues values;
		if (resultCacheEntry == null) {
			MutablePMC pmc = buildAlterablePMCForSteady(dtmc, b1, mcRewards);
