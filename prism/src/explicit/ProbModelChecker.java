...
public class ProbModelChecker extends NonProbModelChecker
{
	 ...
	 */
	public <Value> void exportStateRewardsToFile(Model<Value> model, int r, int exportType, PrismLog out, int precision) throws PrismException
	{
		...
		case DTMC:
		case CTMC:
			exportMCStateRewardsToFile(model, (MCRewards<Value>) modelRewards, exportType, out, precision);
			break;
		case MDP:
		case STPG:
			exportMDPStateRewardsToFile(model, (MDPRewards<Value>) modelRewards, exportType, out, precision);
			break;
		default:
	 ...
	 * @param precision number of significant digits >= 1
	 */
	protected <Value> void exportMCStateRewardsToFile(Model<Value> model, MCRewards<Value> mcRewards, int exportType, PrismLog out, int precision) throws PrismException
	{
	 ...
	 * @param precision number of significant digits >= 1
	 */
	public <Value> void exportMDPStateRewardsToFile(Model<Value> model, MDPRewards<Value> mdpRewards, int exportType, PrismLog out, int precision) throws PrismException
	{
		int numStates = model.getNumStates();
		int nonZeroRews = 0;
