...
public class TypeStruct extends Type 
{
	...
	public boolean canCastTypeTo(Type type)
	{
		if (type instanceof TypeStruct) {
			...
			for (int i = 0; i < numFields; i++) {
				...
				// Field must be castable
				if (!(getFieldType(i).canCastTypeTo(typeStruct.getFieldType(i)))) {
					return false;
				}
			}
			return true;
	...
	@Override
	public List<Object> castValueTo(Object value, EvalMode evalMode) throws PrismLangException
	{
		// Must be a list
		if (!(value instanceof List<?>)) {
			throw new PrismLangException("Can't convert " + value.getClass() + " to type " + getTypeString());
		...
		int numFields = getNumFields();
		for (int i = 0; i < numFields; i++) {
			newList.add(getFieldType(i).castValueTo(oldList.get(i), evalMode));
		}
		return newList;
	}
	...
	{
		if (rhs instanceof TypeStruct) {
			TypeStruct typeStruct = (TypeStruct) rhs;
			int numFields = getNumFields();
			for (int i = 0; i < numFields; i++) {
				// If field name is present, it needs to match
				// (might be missing for a struct literal though)
				...
				getFieldType(i).checkAssignAllowed(typeStruct.getFieldType(i));
			}
		} else {
			throw new PrismLangException("Cannot assign " + rhs + " to " + this);
		}
