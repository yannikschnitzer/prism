...
public class Accuracy
{
	...
	{
		return type == AccuracyType.ABSOLUTE ? errorBound : errorBound * Math.abs(result);
	}
	...
	public double getRelativeErrorBound(double result)
	{
		if (type == AccuracyType.RELATIVE) {
			...
			return errorBound == 0.0 ? 0.0 : errorBound / Math.abs(result);
		}
	}
	
	/**
	 * Get the lower bound for a (specified) result,
	 * factoring in both and error bound and possible floating point error.    
	 */
	public double getResultLowerBound(double result)
	{
		return result - getAbsoluteErrorBound(result) - FLOATING_POINT_ERROR;
	}
