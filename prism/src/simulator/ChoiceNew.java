 ...
 */
public class ChoiceNew<Value> implements Choice<Value>
{
	...
	protected List<List<Pair<Integer,Object>>> updates;
	 ...
	 * Create empty choice.
	 */
	public ChoiceNew(Evaluator<Value> eval)
	{
		// Store evaluator
		this.eval = eval;
		// Initialise
		updates = new ArrayList<List<Pair<Integer,Object>>>();
	 ...
	 */
	public ChoiceNew(ChoiceNew<Value> ch)
	{
		eval = ch.eval;
		moduleOrActionIndex = ch.moduleOrActionIndex;
		updates = new ArrayList<List<Pair<Integer,Object>>>(ch.updates.size());
		for (List<Pair<Integer,Object>> list : ch.updates) {
			List<Pair<Integer,Object>> listNew = new ArrayList<Pair<Integer,Object>>(list.size()); 
			updates.add(listNew);
			for (Pair<Integer,Object> up : list) {
				listNew.add(up);
			}
		}
	 ...
	 */
	public void add(Value probability, List<Pair<Integer,Object>> ups)
	{
		this.updates.add(ups);
		this.probability.add(probability);
	}

	public void add(ChoiceNew<Value> ch)
	{
//		this.probability.add(probability);
		...
		for (int i = 0; i < n; i++) {
			Value p = ch.getProbability(i);
			this.probability.add(p);
//			for (Pair<Integer,Object> pair : ch.updates.get(i)) {
//				
//			}
			this.updates.add(ch.updates.get(i));
	 ...
	 */
	public void productWith(ChoiceNew<Value> ch)
	{
		...
		for (i = 1; i < n; i++) {
			...
			for (j = 0; j < n2; j++) {
				...
				list = new ArrayList<Pair<Integer,Object>>(updates.get(j).size() + ch.updates.get(i).size());
				for (Pair<Integer,Object> u : updates.get(j)) {
					list.add(u);
				}
				for (Pair<Integer,Object> u : ch.updates.get(i)) {
					list.add(u);
				}
		...
		for (j = 0; j < n2; j++) {
			for (Pair<Integer,Object> u : ch.updates.get(0)) {
				updates.get(j).add(u);
			}
	...
	public String getModuleOrAction()
	{
		...
		Pair<Integer,Object> u = updates.get(0).get(0);
		Command c = null;//u.getParent().getParent();
	...
	public String getUpdateString(int i, State currentState) throws PrismLangException
	{
		...
		String s = "";
		boolean first = true;
		for (Pair<Integer,Object> up : updates.get(i)) {
//			n = up.size();
//			for (j = 0; j < n; j++) {
				if (first)
					first = false;
...
//		}
	...
	public State computeTarget(int i, State currentState, VarList varList) throws PrismLangException
	{
		...
		computeTarget(i, currentState, newState, varList);
		return newState;
	}
	...
	public void computeTarget(int i, State currentState, State newState, VarList varList) throws PrismLangException
	{
		...
		for (Pair<Integer,Object> up : updates.get(i))
			newState.setValue(up.getKey(), up.getValue());
	}
