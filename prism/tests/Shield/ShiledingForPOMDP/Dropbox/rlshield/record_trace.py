...
def main():
    ...
    def get_list_for_interger(string):
        string = string[1:-1].split(',')
        return [int(num) for num in string]

    def get_action_by_action_name(state, action_name, max_num_actions):
        action = None
        for possible_action in range(max_num_actions + 1):
            try:
                possible_action_name = recorder._renderer._get_action_string(state, possible_action)
            except:
                pass
            else:
                if possible_action_name == action_name:
                    return possible_action

    def get_actions_by_action_names(state, action_names, max_num_actions):
        if action_names == "notApplicable":
            return range(0,1)
        action_names = action_names[1:-1].split(',')
        actions = []
        for action_name in action_names:
            action_name = action_name.replace("'","").replace(" ", "")
            actions.append(get_action_by_action_name(state, action_name, max_num_actions))
        return actions
    ...
    import pandas as pd
    file_path = "/opt/rlshield/examples/traces_for_video/"
    #import os
    ...
    for file_name in files:
        ...
        trace = pd.read_table(file_name, sep = ';')
        for i in range(len(trace)):
            state = trace.loc[i, "stompy_state"]
            belief_support = trace.loc[i, "stompy_belief_support"]
            available_actions = trace.loc[i, "available_actions"]
            allowed_actions = trace.loc[i, "allowed_actions"]
            selected_action = trace.loc[i, "selected_action"]
            
            print(state)
            print(get_list_for_interger(belief_support),belief_support)
            print("available actions", available_actions, get_actions_by_action_names(state, available_actions, 9))
            print("allowed actions",allowed_actions, get_actions_by_action_names(state, allowed_actions, 9))
            if selected_action != "notApplicable":
                print("selected action",selected_action)#, get_action_by_action_name(state, selected_action, 9))
            
            recorder.record_state(state)
            recorder.record_belief(get_list_for_interger(belief_support))
            recorder.record_available_actions(get_actions_by_action_names(state, available_actions, 9))
            recorder.record_allowed_actions(get_actions_by_action_names(state, allowed_actions, 9))
            if selected_action != "notApplicable":
                recorder.record_selected_action(get_action_by_action_name(state, selected_action, 9))


        recorder.end_path(True)
        recorder.save(file_path, file_name)
