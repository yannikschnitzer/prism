                ...
                if (qualSettings.isMemlessSearchSet()) {
                    ...
                    if (qualSettings.getMemlessSearchMethod() == "one-shot") {
                        ...
                        if (qualSettings.isExportWinningRegionSet()) {
                            ...
                            sstr << "1--------------------add to orginal"<<std::endl;
                            
                            std::ofstream file;
                            storm::utility::openFile(qualSettings.exportWinningRegionPath(), file, true);
                            file << ":state information"<<std::endl;
                            for (uint64_t state = 0; state < pomdp.getNumberOfStates(); ++state) {
                                file << origpomdp->getStateValuations().getStateInfo(state) << " & state="<< state<< " & obs=" << origpomdp->getObservation(state) <<  std::endl;
                            }
                            storm::utility::closeFile(file);
                            /*
                            sstr << "2--------------------add to another function"<<std::endl;                            

                            origpomdp->printModelInformationToFile(qualSettings.exportWinningRegionPath().append("test2"));

                            sstr << "3---------------------add to current function"<<std::endl;

                            std::ofstream fileB;
                            storm::utility::openFile(qualSettings.exportWinningRegionPath().append("D"), fileB, true);
                            for (uint64_t state = 0; state < pomdp.getNumberOfStates(); ++state) {
                                fileB << origpomdp->getStateValuations().getStateInfo(state) << " & state="<< state<< " & obs=" << origpomdp->getObservation(state) <<  std::endl;
                            }
                            storm::utility::closeFile(fileB);
