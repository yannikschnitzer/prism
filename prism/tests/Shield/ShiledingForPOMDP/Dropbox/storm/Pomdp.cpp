        ...
        namespace sparse {
            ...
            void Pomdp<ValueType, RewardModelType>::printModelInformationToStream(std::ostream& out) const {
                ...
                for (uint_fast64_t state = 0, highestStateIndex = this->getNumberOfStates() - 1; state <= highestStateIndex; ++state) {
                    out << this->getStateValuations().getStateInfo(state) << " & state="<< state<< " & obs=" << this->getObservation(state) <<  std::endl;
                }
            ...
            ////////////////////////////////added
            template<typename ValueType, typename RewardModelType>
            void Pomdp<ValueType, RewardModelType>::printModelInformationToFile(std::string const& path) const {
                std::ofstream file;
                storm::utility::openFile(path, file, true);
                for (uint_fast64_t state = 0, highestStateIndex = this->getNumberOfStates() - 1; state <= highestStateIndex; ++state) {
                    file << this->getStateValuations().getStateInfo(state) << " & state="<< state<< " & obs=" << this->getObservation(state) <<  std::endl;
                }
