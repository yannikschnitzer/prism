...
const int N = 1; // number of inputs in each bundle
...
module multiplex
	...
	u : [1..M] init 2; // number of stages
	c : [0..N]; // counter (number of copies of the nand done)

	s : [0..4] init 1; // local state
	// 0 - initial state
	// 1 - set x inputs
	// 2 - set y inputs
	// 3 - set outputs
	...
	[] u=3 & zx=0 & goal=0 -> p : (goal'=1) + (1-p) : (goal'=2);
	[] u=3 & zx=1 & goal=0 -> q : (goal'=1) + (1-q) : (goal'=2);
	...
	[] s=1 & u>1 & u<3 & zx>0 -> (x'=1) & (s'=2) & (zx'=zx-1);
	[] s=1 & u>1 & u<3 & zx=0 -> (x'=0) & (s'=2);

	// choose x randomly from selection (have zy stimulated inputs)
	[] s=2 & u=1 -> prob1 : (y'=1) & (s'=3) + (1-prob1) : (y'=0) & (s'=3); // initially random
	[] s=2 & u>1 & u<3 & zy<(N-c) & zy>0  -> zy/(N-c) : (y'=1) & (s'=3) & (zy'=zy-1) + 1-(zy/(N-c)) : (y'=0) & (s'=3);
	[] s=2 & u>1 & u<3 & zy=(N-c) & c<N -> 1 : (y'=1) & (s'=3) & (zy'=zy-1);
	[] s=2 & u>1 & u<3 & zy=0 -> 1 : (y'=0) & (s'=3);
