        ...
        for alg in alg_types:
            if '-' in alg:
                ...
                for rep in rep_types:
                    print(f'\nRunning uncertain base experiment:{exp}, alg:{alg}, rep:{rep}')
    ...
    for exp in all_experiments:
        for alg in alg_types: # imin imax
            if '-' in alg:
                ...
                for rep in rep_types:
                    print(f'\nRunning Robust VI experiment:{exp}, alg:{alg}, rep:{rep}')
                    ...
                    if r==0:
                        # save output for current algorithm
                        print(f"--- All done : {alg}  -- Experiment: {exp}")
                    else:
                        print(f'Error running experiment')
                        # copy_log_files(cmd_base_copy, experiment_folder, exp, alg, rep, apdx, prefix, debug, isUncertain)
...
config = {
    ...
    'betting_g_gran_low' :{'model':prefix+'tests/betting_game/betting_game_gran_low.prism', 'props':prefix+'tests/betting_game/betting_game.props', 'dir':'betting_game/', 'pn':[3,2,-1,1,4,5], 'vmax': def_vmax, 'atoms':201, 'epsilon':def_eps, 'b':101, 'alpha':0.8, 'u_atoms':11, 'u_bounds':[0.0, 1.0], 'params':['p_11']},
    'betting_g_gran_high' :{'model':prefix+'tests/betting_game/betting_game_gran_high.prism', 'props':prefix+'tests/betting_game/betting_game.props', 'dir':'betting_game/', 'pn':[3,2,-1,1,4,5], 'vmax': def_vmax, 'atoms':201, 'epsilon':def_eps, 'b':101, 'alpha':0.8, 'u_atoms':41, 'u_bounds':[0.0, 1.0], 'params':['p_41']},
...
prob_experiments = ['underwater_tiny','firewire','betting_g','coin2', 'drone_small', 'coin2_mid']
